      ...
      <body className="bg-zinc-950 text-zinc-100">
        ...
        <section id="contact" className="max-w-4xl mx-auto px-6 py-20">
          ...
          <form onSubmit={sendEmail} className="grid gap-4">
            <input name="name" required placeholder="Name" className="p-3 bg-zinc-900 rounded" />
            <input name="email" type="email" required placeholder="Email" className="p-3 bg-zinc-900 rounded" />
            <textarea name="message" required placeholder="Message" rows="4" className="p-3 bg-zinc-900 rounded" />
            <button disabled={loading} className="bg-cyan-500 text-zinc-900 py-2 rounded">{loading ? 'Sending...' : 'Send Message'}</button>
            {status && <p className="text-sm">{status}</p>}
          </form>
        </section>

        <footer className="text-center py-6 text-zinc-500">© {new Date().getFullYear()} Md. Atik Hasan</footer>
      </body>
    </html>
  );
}
